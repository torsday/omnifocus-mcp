...
4. **Cap** (`cap.ts`, [#776](https://github.com/torsday/omnifocus-mcp/issues/776) / [#1059](https://github.com/torsday/omnifocus-mcp/issues/1059)) — `applyByteCap` enforces an optional `maxOutputBytes` wire-byte ceiling, trimming whole items and re-anchoring the continuation cursor at the last kept one. Last so it sees the post-elide post-truncate size. Wired into the **cursor-paginated** list reads: `task_list`, `search_query`, `project_list` (each supplies a `cursorFor` callback that resumes at the first dropped item — see `taskService.cursorForListItem` / `searchService.cursorForResultItem` / `projectService.cursorForListItem`). Non-cursor/bounded reads (`tag_list`, `forecast_get`, `*_get_many`) need a different no-cursor truncation model — tracked as a follow-up.
