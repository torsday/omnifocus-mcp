...
- **Every workflow runs on the self-hosted `mac-local` runner.** Labels: `self-hosted, macOS, ARM64, macos-omnifocus`. All ten workflows (`ci.yml`, `release.yml`, `integration.yml`, `board-sync.yml`, `issue-lint.yml`, `pr-link.yml`, `pr-title.yml`, `meta-lint.yml`, `validate-deps-nightly.yml`, `verify-constants.yml`) target `[self-hosted, macos]`. We pay nothing for runner minutes; GitHub-hosted billing state is irrelevant to PR CI. `meta-lint.yml`'s `no-hosted-runners` job is a regression guard — `scripts/verify-no-hosted-runners.sh` fails the build if any workflow re-introduces `ubuntu-latest` / `macos-latest` / `windows-latest`. This project is macOS-only (OmniFocus is macOS-only — JXA/OmniJS don't exist on Linux), no external contributors run untrusted code, and concurrency demand is one-PR-at-a-time, so GitHub-hosted gives nothing we need.
