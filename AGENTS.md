...
- **Runner policy — two tiers:**
  - `integration.yml`'s heavy job runs on `[self-hosted, macos-omnifocus]` (labels: `self-hosted, macOS, ARM64, macos-omnifocus`). OmniFocus access is required — there is no substitute. The `integration-gate` job is on `ubuntu-latest` so the required check is reachable even when the self-hosted runner is offline (gap 2 of [#679](https://github.com/torsday/omnifocus-mcp/issues/679)).
  - `ci.yml`'s `build (Node 24)` runs on `ubuntu-latest` with a `# allow-hosted` line marker. It exercises only the InMemoryAdapter — no JXA or OmniJS calls — so OS parity is not load-bearing for what it verifies. Keeping CI on the self-hosted runner forced it to share queue with the integration suite; a single hung integration test would block every queued PR's required `build (Node 24)` check for hours. Moved to ubuntu-latest as part of the same 2026-05-10 push that added the runner-side watchdog and `timeout-minutes: 30` cap to `integration.yml`.
  ...
  - `meta-lint.yml`'s `no-hosted-runners` job enforces the policy — `scripts/verify-no-hosted-runners.sh` fails the build if any non-allowlisted workflow uses a GitHub-hosted runner without a `# allow-hosted` line marker. The allowlist + per-line markers are the canonical record of exceptions.
...
- **JXA bridge contention with concurrent MCP clients** (#932) — the JXA bridge is single-threaded, so each active Claude-client-spawned MCP server queues against the same bridge the integration suite uses. With 5+ Claude clients running, integration jobs time out at the 60s `osascript` cap before the seed query starts executing. `integration-gate` is soft-failed in `release.yml` (#933) while [ADR-0023](./docs/adr/0023-runner-host-bridge-contention.md) ships; until a dedicated runner exists, merge release-please PRs only when no interactive Claude session is active. Failure signature: `Seed script failed: osascript spawn failed: spawnSync osascript ETIMEDOUT`. See [`docs/runner-setup.md` §5](./docs/runner-setup.md#5-jxa-bridge-contention-with-concurrent-mcp-clients) for the operational pre-flight.
