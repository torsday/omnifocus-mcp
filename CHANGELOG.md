...
**Summary** — Three new capability areas: project templates, waiting-on tracking, and custom perspective management. Alongside these, a full NL-quality pass landed across the entire tool surface — every tool description now carries a worked `Example:` line, response objects pair human-readable names with opaque IDs, and enum inputs accept common aliases (`"done"` alongside `"completed"`, etc.). Agents calling this server cold for the first time will be able to compose calls with substantially less trial-and-error. Bundle size budget bumped 540→580 KiB to accommodate the new tools (still well inside the limit; see `scripts/check-bundle-size.sh`). No breaking changes; all existing tool call shapes are unchanged.
...
- **Project templates — save + list + instantiate** — `project_template_save` captures a project's task tree as TaskPaper into a new project under the configured `Templates` folder (env `OMNIFOCUS_TEMPLATES_FOLDER_NAME`, default `Templates`). Template metadata and `{{placeholder}}` parameter names sit in a fenced YAML block in the template project's note. `project_template_list` enumerates every parseable template (sorted by capturedAt desc). `project_template_instantiate` resolves a template by name, validates all declared parameters are supplied, substitutes `{{name}}` placeholders, and shifts every `@due`/`@defer` date by the delta between the template's earliest `@due` and a caller-supplied `dueDate`. Returns `{ projectId, taskCount, importWarnings }`. Duplicate template names and missing parameters produce typed errors. Convention documented in DESIGN.md §30. ([#472](https://github.com/torsday/omnifocus-mcp/issues/472), [#587](https://github.com/torsday/omnifocus-mcp/issues/587))
- **`*_describe` preview tools for every write operation** — dry-run companion for every mutating tool (`task_create_describe`, `project_create_describe`, etc.) returns a human-readable summary of what the live call would do without touching OmniFocus. Lets an agent confirm its intent before firing a mutation. ([#494](https://github.com/torsday/omnifocus-mcp/issues/494))
- **`app_window_new` and `app_window_new_tab`** — open a new OmniFocus window or a new tab on the frontmost window via OmniJS. Useful for guided-review flows that want to surface a fresh Forecast or Projects view without displacing the user's current context. ([#527](https://github.com/torsday/omnifocus-mcp/issues/527))

### Improved

- **NL-quality: `Example:` line on every tool description** — all ~95 `*_DESCRIPTION` constants now end with at least one `Example: tool_name({ … })` line; multi-mode tools (e.g. `task_list`, `project_list`) include 2–3 representative examples. Agents reading the tool list can now construct a first call from the description alone without additional prompting. ([#570](https://github.com/torsday/omnifocus-mcp/issues/570))
- **NL-quality: name paired with id in all mutation responses** — every tool that previously returned a bare `{ id }` now returns `{ id, name }` (or the domain equivalent). Agents can confirm what they just created/modified without a follow-up `*_get` call. Covers task verbs, project verbs, batch task and project operations, attachment operations, forecast tag operations, and note operations. ([#572](https://github.com/torsday/omnifocus-mcp/issues/572), [#585](https://github.com/torsday/omnifocus-mcp/issues/585), [#590](https://github.com/torsday/omnifocus-mcp/issues/590), [#592](https://github.com/torsday/omnifocus-mcp/issues/592), [#597](https://github.com/torsday/omnifocus-mcp/issues/597), [#606](https://github.com/torsday/omnifocus-mcp/issues/606))
- **NL-quality: forgiving enum aliases** — status, completion-criterion, and related enum inputs now accept common synonyms (`"done"` / `"dropped"` / `"active"` alongside the canonical values). Mis-typed or loosely-worded agent inputs are accepted rather than rejected with a validation error. ([#566](https://github.com/torsday/omnifocus-mcp/issues/566))
- **NL-quality: `zodToActionable` boundary error rewriting** — Zod validation failures at the tool boundary are rewritten into structured, agent-actionable error messages that name the failing field and suggest the accepted value set, instead of raw Zod issue arrays. ([#489](https://github.com/torsday/omnifocus-mcp/issues/489))

### Documentation

- **ADR-0018 — Calendar bridge: EventKit only, Swift-binary subprocess** — formalises the architecture for the upcoming calendar integration (#484). Decisions: EventKit only (no third-party calendar APIs); access via a small bundled Swift binary subprocess; read-only; permission UX mirrors the existing OF Automation prompt. ([#603](https://github.com/torsday/omnifocus-mcp/issues/603))
- **Setup guides for OpenCode and Pi** — `docs/clients/opencode.md` documents OpenCode's `~/.config/opencode/opencode.json` MCP config format. `docs/clients/pi.md` honestly records that Pi (pi.ai) has no MCP client support as of mid-2025 and redirects to supported clients. ([#559](https://github.com/torsday/omnifocus-mcp/issues/559))
- **Homebrew tap formula** — `torsday/homebrew-tap` now carries `Formula/omnifocus-mcp.rb`; the release workflow patches it automatically after each npm publish. Non-Node users can install and upgrade via `brew install torsday/tap/omnifocus-mcp`. ([#534](https://github.com/torsday/omnifocus-mcp/issues/534))
