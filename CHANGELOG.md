...
- **`perspective_evaluate_dry_run` — preview a proposed rule tree without persisting it (closes #659)** — new tool that takes `{ aggregation?, rules }` and returns `{ tasks: Task[] }` for the proposed perspective. Implementation creates a temporary perspective with a sentinel name (`__omnifocus-mcp-dry-run-<timestamp>-<rand>`), evaluates it, and **always** deletes the temp inside one OmniJS execution — a transport-level retry between hops can never leave an orphan. Reuses `perspective_create`'s `PerspectiveRuleInputSchema` so a successful dry-run's rule tree can be passed verbatim to the persistent create call. Custom perspectives require OmniFocus Pro (returns `OF_FEATURE_REQUIRES_PRO`). Unblocks [#476](https://github.com/torsday/omnifocus-mcp/issues/476)'s perspective-author prompt: propose rules → preview matched tasks via dry-run → commit via `perspective_create`. Closes [#659](https://github.com/torsday/omnifocus-mcp/issues/659).
