# ADR-0001: TypeScript on Node.js 24 as the implementation stack
...
We will build on **TypeScript 6.x targeting Node.js 24+**, using `@modelcontextprotocol/sdk` for MCP transport. Node 24 is the floor (`engines.node: ">=24"` in `package.json`); the bundle is built with `tsup`'s `target: "node24"` so we can use modern syntax without polyfills.
