...
- **Deletions deferred (v1).** `removed` is **not** reported — there is no deletion signal. The tool description and this ADR state the limitation; consumers reconcile periodically against `task_list` / `project_list`. Tracked by a follow-up issue.
...
- **Implement `removed` via full-ID reconciliation each call** — rejected for v1: it requires enumerating all IDs every poll (the cost the delta protocol exists to avoid). Deferred to a follow-up that can pair it with a watcher-level deletion signal.
