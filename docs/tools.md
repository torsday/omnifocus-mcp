...
| `nextReviewDate` | unknown | Yes | Next review date as ISO-8601 (with offset). Pass null to reset to the interval-derived schedule — OmniFocus recomputes the date from last review date + review interval and cannot leave a project unscheduled. Past-dated values are accepted and mark the project as overdue immediately. |
